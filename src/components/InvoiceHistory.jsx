...
const InvoiceHistory = () => {
    ...
    const showToast = (message, type = "success") => {
        ...
        } else if ((type === "error" || type === "warning") && errorSoundRef.current) {
    ...
    const fetchInvoices = useCallback(async () => {
        try {
            ...
            if (res.data.success) {
                ...
                showToast(res.data.message || "Failed to load invoice history.", "error");
            }
        } catch (err) {
            console.log(err);
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (err.response?.status === 401) {
                ...
                showToast("Only Admin can delete invoices.", "error");
            } else {
                            ...
                            <div className="flex justify-center mb-5">
                                <div className="w-14 h-14 rounded-full bg-red-50 border border-red-100 flex items-center justify-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-7 h-7 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01M10.29 3.86l-7.82 14A2 2 0 004.21 21h15.58a2 2 0 001.74-3.14l-7.82-14a2 2 0 00-3.42 0z" />
                                    </svg>
                                </div>
                            </div>
                            <h2 className="text-xl font-bold text-slate-800 text-center">Delete Invoice?</h2>
                            <p className="text-sm text-slate-500 text-center mt-2 leading-6">Are you sure you want to delete this invoice?</p>
                            <p className="text-xs text-red-500 text-center mt-2">This action cannot be undone.</p>
                            <div className="grid grid-cols-2 gap-3 mt-6">
                                <button type="button" onClick={cancelDelete} className="h-11 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-sm transition">Cancel</button>
                                <button type="button" onClick={handleDelete} className="h-11 rounded-lg bg-red-600 hover:bg-red-700 text-white font-semibold text-sm transition">Delete</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                ...
                <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
                    <div className="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl shadow-sm">
                        ...
                        <span className="text-sm font-semibold text-slate-700">{filteredInvoices.length} Invoice{filteredInvoices.length !== 1 ? "s" : ""}</span>
                    </div>
                    <button type="button" onClick={() => navigate("/refund-history")} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-sm font-semibold transition shadow-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 14l-4-4 4-4" />
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10h9a5 5 0 015 5v1" />
                        </svg>
                        Refund History
                    </button>
                </div>
                ...
                <div className="px-5 sm:px-6 py-5 border-b border-slate-200">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div className="relative w-full sm:w-80">
                            ...
                            />
                            {search && (
                                <button type="button" onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-700 text-lg transition">×</button>
                            )}
                        </div>
                    </div>
                </div>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filteredInvoices.length > 0 ? (
                                    filteredInvoices.map((invoice) => {
                                        const method = invoice.payment_method || invoice.payment_Method || "";
                                        const isSplit = String(method).toLowerCase() === "split";
                                                    ...
                                                    {isSplit ? (
                                                        <div className="inline-flex flex-col items-center">
                                                            ...
                                                            <span className="text-[10px] text-slate-500 mt-0.5 whitespace-nowrap">
                                                                C: ₹{Number(invoice.cash_amount || 0).toFixed(0)} | O: ₹{Number(invoice.online_amount || 0).toFixed(0)}
                                                            </span>
                                                        </div>
                                                    ) : isCash ? (
                                                        <span className="inline-flex px-2.5 py-1 rounded-md text-xs font-medium bg-emerald-50 border border-emerald-200 text-emerald-700">
                                                            Cash
                                                        </span>
                                                    ) : (
                                                        <span className="inline-flex px-2.5 py-1 rounded-md text-xs font-medium bg-blue-50 border border-blue-200 text-blue-700">
                                                            {method || "Online"}
                                                        </span>
                                                    )}
                                                </td>
